...
function evaluateClearStatus(result, type) {
  ...
  // 2. 文字数の目標値（制限時間で変動し、自己ベストを超えるとさらに上がる）
  let targetChars = 100;
  if (timeLimit) {
  ...
  switch (type) {
    ...
    case 'philanthropist':
      // 利他主義者: 制限時間に応じた文字数を貢献（自己ベストを超えるとさらに上がる）
    ...
    case 'freeSpirit':
    default:
      // 自由人: 時間に応じた完了文数（自己ベストを超えるとさらに上がる）
...
const DEFAULT_GROUP = 'A'; // このページ(index.html)はA群（AIチャット診断）固定
...
// その場合は即座に初期化する（診断なし版・アンケート版でも同じパターン）。
function bootApp() {
  initParticipantInfo();
  initWelcomeScreen();
